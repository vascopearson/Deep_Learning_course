...
class LogisticRegression(nn.Module):
    ...
    def __init__(self, n_classes, n_features, **kwargs):
        ...
        self.w1 = nn.Linear(n_features, n_classes) #bias is true by default
...
class FeedforwardNetwork(nn.Module):
    ...
    def forward(self, x, **kwargs):
        ...
        for layer in self.w:
            z = layer(h)
            h_init = self.dropout(z)
            h = self.activation(h_init)
        out = self.out(h)
        return out
...
def train_batch(X, y, model, optimizer, criterion, **kwargs):
    ...
    loss = criterion(outputs, y) #Compute loss function
    
    loss.backward() #Computes the gradient of the given tensor w.r.t. the weights/bias
    
    optimizer.step() #Updates weights and biases with the optimizer (SGD or Adam)
