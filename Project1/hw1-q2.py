...
def solve_analytically(X, y):
    w = np.linalg.inv(X.transpose().dot(X) + np.multiply(np.identity(X.transpose().shape[0]), 10**-4)).dot(X.transpose()).dot(y)
    return w
    """
...
class LinearRegression(_RegressionModel):
    ...
    def update_weight(self, x_i, y_i, learning_rate=0.001):
        ...
        for j in range(len(x_i)-1):
            self.w[j+1] -= learning_rate * (error) * x_i[j+1]
...
class NeuralRegression(_RegressionModel):
    ...
    def update_weight(self, x_i, y_i, learning_rate=0.001):
        ...
        """
        #Forward pass
        z_1 = (self.w1.dot(x_i) + self.b1).T
        h_1 = np.maximum(z_1, 0)
        yhat = (self.w2.dot(h_1) + self.b2)#[0][0]
        
        #Backpropagation
        grad_yhat = (yhat - y_i)
        grad_W2 = (np.multiply(grad_yhat, h_1)).T
        grad_b2 = grad_yhat
        grad_h1 = np.multiply(self.w2, grad_yhat)
        grad_z1 = np.multiply(grad_h1.T, np.where(z_1 <= 0, 0, 1))
        grad_W1 = np.dot(grad_z1, np.reshape(x_i, (1,-1)))
        grad_b1 = grad_z1.T
    ...
    def predict(self, X):
        ...
        for i in range(X.shape[0]):
            z_1 = (np.dot(self.w1, X[i].T) + self.b1).T
            h_1 = np.maximum(0, z_1)
            yhat[i] = np.dot(self.w2, h_1) + self.b2
        return yhat
